...
namespace SmartStore.Services.Authentication
{
    public partial class FormsAuthenticationService : IAuthenticationService
    {
		...
		public Google2FASetup SetupAuth(Customer customer)
		{
			TwoFactorAuthenticator tfa = new TwoFactorAuthenticator();
			var setupInfo = tfa.GenerateSetupCode("GlobalCryptoTeam", customer.Email, customer.Email + "yVWiHjV1jaK0mGcbtYSX",false,150);
			Google2FASetup google2FASetup = new Google2FASetup()
			{
				qrCodeImageUrl = setupInfo.QrCodeSetupImageUrl,
				manualEntrySetupCode = setupInfo.ManualEntryKey
			};
			return google2FASetup;
		}
		public bool Validated2FA(Customer customer,string pin)
		{
			TwoFactorAuthenticator tfa = new TwoFactorAuthenticator();
			var timespan = TimeSpan.FromMinutes(5);
			bool isCorrectPIN = tfa.ValidateTwoFactorPIN(customer.Email + "yVWiHjV1jaK0mGcbtYSX", pin);
			return isCorrectPIN;
		}
    }
}